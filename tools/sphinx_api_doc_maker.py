...
def main(
    ...
    Main function.
    '''
    doc_dir = os.path.abspath(os.path.join(word2vec_mt.path, '..', '..', 'docs', 'source'))

    try:
        os.remove(os.path.join(doc_dir, 'word2vec_mt.rst'))
    except FileNotFoundError:
        pass

    try:
        shutil.rmtree(os.path.join(doc_dir, 'word2vec_mt'))
    except FileNotFoundError:
        pass
    ...
    with open(
        ...
        print('''\
Welcome to word2vec_mt's documentation!
=======================================
...
.. toctree::
    ...
    ''' + (
       '' if set(os.listdir(os.path.join(word2vec_mt.path))) - {'__pycache__'} == {'__init__.py'}
       else 'word2vec_mt'
   ) + '''
''', file=f)
    ...
    generate(
        os.path.abspath(os.path.join(word2vec_mt.path, '..')),
        doc_dir,
        [],
    )
